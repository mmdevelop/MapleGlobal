...
** Location: Perion: West Rocky Mountain IV
** Purpose: 2nd Job Advancer
...
{//getJob() returns job ID
	...
	if(cm.getJob() == 100 && cm.getLevel() >= 30 && cm.haveItem(4031008, 1, false, true) && have29OrLess4031013 && cm.haveItem(4031012, 0))
	{
		if(status == -1)
		{
		...
		else if(status == 3)
		{
			if(mode == 1)//i.e. they hit Yes
				...
				{
					cm.getPlayer().changeMap(108000300)
				...
				{
					cm.getPlayer().changeMap(108000301)
					cm.sendOk("Defeat the monsters inside, collect 30 Dark Marbles, then strike up a conversation with a colleague of mine inside. He'll give you #bThe Proof of a Hero#k, the proof that you've passed the test. Best of luck to you.")
					cm.dispose()
				}
				else if(cm.getPlayerCount(108000302) == 0)
				{
					cm.getPlayer().changeMap(108000302)
					cm.sendOk("Defeat the monsters inside, collect 30 Dark Marbles, then strike up a conversation with a colleague of mine inside. He'll give you #bThe Proof of a Hero#k, the proof that you've passed the test. Best of luck to you.")
					cm.dispose()
				}
				else
