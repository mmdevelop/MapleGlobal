...
function start()
{
	...
	{
		cm.sendNext("Make sure you have a #bTicket to Orbis#k to travel on this ship.");
		cm.dispose();
	}
	
	else if (em.getProperty("entry") == "true" && cm.getPlayer().getMap().hasBoat() == 2)
	{
		cm.sendYesNo("It looks like there's plenty of room for the ship to Orbis. Please have your ticket ready so I can let you in. This will not be a short flight, so if you need to take care of some things, I suggest you do that first before getting on board. Do you still wish to board the ship?");
	}
...
function action(mode)
{
	...
	else if (mode == 1)
	{
		...
		cm.getPlayer().changeMap(101000301);
		cm.dispose();
	}
}