...
** Location: The Road to the Dungeon - Warning Street
** Purpose: 2nd Job Advancer: Bowman
...
{//getJob() returns job ID
	...
	if(cm.getJob() == 300 && cm.getLevel() >= 30 && cm.haveItem(4031010, 1, false, true) && have29OrLess4031013 && cm.haveItem(4031012, 0))
	{
		if(status == -1)
		{
			cm.sendNext("Hmmm...it is definitely the letter from #bAthena Pierce#k...so you came all the way here to take the test and make the 2nd job advancement as the Bowman. Alright, I'll explain the test to you. Don't sweat it too much, it's not that complicated.")
		...
		else if(status == 3)
		{
			if(mode == 1)//i.e. they hit Yes
				if(cm.getPlayerCount(108000100) == 0)
				{
					cm.getPlayer().changeMap(108000100)
				...
				else if(cm.getPlayerCount(108000101) == 0)
				{
					cm.getPlayer().changeMap(108000101)
					cm.sendOk("Defeat the monsters inside, collect 30 Dark Marbles, then strike up a conversation with a colleague of mine inside. He'll give you #bThe Proof of a Hero#k, the proof that you've passed the test. Best of luck to you.")
					cm.dispose()
				}
				else if(cm.getPlayerCount(108000102) == 0)
				{
					cm.getPlayer().changeMap(108000102)
					cm.sendOk("Defeat the monsters inside, collect 30 Dark Marbles, then strike up a conversation with a colleague of mine inside. He'll give you #bThe Proof of a Hero#k, the proof that you've passed the test. Best of luck to you.")
					cm.dispose()
				}
				else
	...
	{
		cm.sendOk("Hello there. Do you want something from me?...")
		cm.dispose()
	}
}