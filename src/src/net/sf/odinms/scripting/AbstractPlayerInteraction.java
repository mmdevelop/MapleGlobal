...
public class AbstractPlayerInteraction {
    ...
    public boolean checkQuestStatus(int id, int status) {
       MapleQuestStatus.Status qstatus = getPlayer().getQuest(MapleQuest.getInstance(id)).getStatus();
       return status == qstatus.getId();
    }
    
    public boolean checkQuestData(int id, String data) {
    	return getPlayer().getQuest(MapleQuest.getInstance(id)).getData().equals(data);
    }
    
    public void setQuestData(int id, String data) {
    	getPlayer().getQuest(MapleQuest.getInstance(id)).setData(data);
    	c.announce(CWvsContext.Message.QuestRecordMessage.update(getPlayer().getQuest(MapleQuest.getInstance(id))));
    }
    ...
    public boolean gainItem(int id, short quantity, boolean randomStats, boolean show) {
        if (quantity >= 0) {
            ...
            if (type.equals(MapleInventoryType.EQUIP) && !ii.isThrowingStar(item.getItemId())) {
                ...
                if (id >= 5000000 && id <= 5000100) {
                    ...
                    if (show) {
                        c.announce(MaplePacketCreator.getShowItemGain(id, quantity));
                    }
                } else {
            ...
            if (show) {
            	c.announce(MaplePacketCreator.getShowItemLost(id, quantity));
            }
        }
        return true;
    }
