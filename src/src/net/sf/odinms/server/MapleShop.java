...
public class MapleShop {
    ...
    public void sendShop(MapleClient c) {
        ...
        c.announce(NpcPacket.getNPCShop(getNpcId(), items));
    }
    ...
    public void buy(MapleClient c, int itemId, short quantity) {
        ...
        if (item != null && item.getPrice() > 0) {
            if (c.getPlayer().getMeso() > item.getPrice() * quantity) {
                ...
                c.announce(NpcPacket.confirmShopTransaction((byte) 0));
            }
        }
    }
    ...
    public void sell(MapleClient c, MapleInventoryType type, byte slot, short quantity) {
        ...
        if (quantity <= iQuant && iQuant > 0) {
            ...
            c.announce(NpcPacket.confirmShopTransaction((byte) 0x8));
        }
    }
    ...
    public static MapleShop createFromDB(int npcshopid, boolean isShopId) {
        MapleShop ret = null;
        MapleItemInformationProvider ii = MapleItemInformationProvider.getInstance();
        int shopId;
        try {
