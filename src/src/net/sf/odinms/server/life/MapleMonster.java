...
public class MapleMonster extends AbstractLoadedMapleLife {
    ...
    public ArrayList<Pair<Integer, Integer>> getDrops() {
        ArrayList<Pair<Integer, Integer>> drops = new ArrayList<>();
        MapleMonsterInformationProvider mi = MapleMonsterInformationProvider.getInstance();
        List<DropEntry> dropList = mi.retrieveDropChances(getId());
        
        for (DropEntry drop : dropList)
        {
            if (Math.random() <= drop.chance)
            {
                Pair<Integer, Integer> successfulDrop;
                
                int amount;
                if(drop.money)
                {
                    amount = drop.meso;
                    successfulDrop = new Pair(drop.itemId, amount);
                }
                else if (drop.isStackable)
                {
                    amount = (int)((drop.maxAmount-drop.minAmount)*Math.random())+drop.minAmount + 1;
                    successfulDrop = new Pair(drop.itemId, amount);
                }    
                else
                {
...
//            System.out.println("item: " + d.toString());
//            System.out.println("assigned range start: " + d.assignedRangeStart);
//            System.out.println("assigned range length: " + d.assignedRangeLength);
//        }
//        Random r = new Random();
