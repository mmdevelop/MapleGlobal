...
public class ChannelServer implements Runnable, ChannelServerMBean {
	...
	private ChannelServer(String key) {
		mapFactory = new MapleMapFactory(
				MapleDataProviderFactory.getDataProvider(new File("wz" + "/Map")),
				MapleDataProviderFactory
						.getDataProvider(new File("wz" + "/String")));
		this.key = key;
	}
				...
				synchronized (wci) {
					...
					try {
						...
						Registry registry = LocateRegistry.getRegistry(
								initialProp.getProperty("net.sf.odinms.world.host"), Registry.REGISTRY_PORT,
						...
						dropUndroppables = Boolean
								.parseBoolean(props.getProperty("net.sf.odinms.world.alldrop", "false"));
						moreThanOne = Boolean
								.parseBoolean(props.getProperty("net.sf.odinms.world.morethanone", "false"));
	...
	public void run() {
		...
		// cfg.setThreadModel(ThreadModel.MANUAL); // *fingers crossed*, I hope
		// the executor filter handles everything
		// executor = new ThreadPoolExecutor(16, 16, 60, TimeUnit.SECONDS, new
		...
		try {
			MapleServerHandler serverHandler = new MapleServerHandler(
					PacketProcessor.getProcessor(PacketProcessor.Mode.CHANNELSERVER), channel);
	...
	public static ChannelServer newInstance(String key) throws InstanceAlreadyExistsException,
		...
		mBeanServer.registerMBean(instance,
				new ObjectName("net.sf.odinms.net.channel:type=ChannelServer,name=ChannelServer" + uniqueID++));
	...
	public void setServerMessage(String newMessage) {
		...
		broadcastPacket(CWvsContext.BroadcastMsg.sendServerMessage(serverMessage));
	}
	...
	public void shutdown(int time) {
		broadcastPacket(CWvsContext.BroadcastMsg.sendNotice("The world will be shut down in " + (time / 60000) + " minutes, please log off safely"));
	...
	public static void main(String args[])
			...
			MBeanRegistrationException, NotCompliantMBeanException, MalformedObjectNameException {
		initialProp = new Properties();
		initialProp.load(new FileReader(/*System.getProperty("net.sf.odinms.channel.config")*/"channel.properties"));
