...
public class ChannelWorldInterfaceImpl extends UnicastRemoteObject implements ChannelWorldInterface {
	...
	public void whisper(String sender, String target, int channel, String message) throws RemoteException {
		if (isConnected(target)) {
			server.getPlayerStorage().getCharacterByName(target).getClient().getSession()
					.write(MaplePacketCreator.getWhisper(sender, channel, message));
		}
	}
	...
	public void shutdown(int time) throws RemoteException {
		server.broadcastPacket(CWvsContext.BroadcastMsg.sendNotice("The world will be shut down in " + (time / 60000) + " minutes, please log off safely"));
		...
		for (int buddy : buddies) {
			...
			if (chr != null) {
				...
				if (ble != null && ble.isVisible()) {
					...
					chr.getClient().announce(CWvsContext.FriendResult.friendChangeChannel(ble.getCharacterId(), mcChannel));
				}
			}
		}
	}
	...
	public void updateParty(MapleParty party, PartyOperation operation, MaplePartyCharacter target)
		...
		for (MaplePartyCharacter partychar : party.getMembers()) {
			if (partychar.getChannel() == server.getChannel()) {
				MapleCharacter chr = server.getPlayerStorage().getCharacterById(partychar.getId());
				if (chr != null) {
					if (operation == PartyOperation.DISBAND) {
						chr.setParty(null);
						chr.getClient().announce(CWvsContext.PartyResult.disbandParty(party, partychar));
                                                continue;
					} else {
						chr.setParty(party);
					}
	...
	public void partyChat(MapleParty party, String chattext, String namefrom) throws RemoteException {
		for (MaplePartyCharacter partychar : party.getMembers()) {
			if (partychar.getChannel() == server.getChannel() && !(partychar.getName().equals(namefrom))) {
				...
				if (chr != null) {
					chr.getClient().announce(CField.GroupMessage.PartyChat.chat(namefrom, chattext));
				}
			}
		}
	}
	...
	public List<CheaterData> getCheaters() throws RemoteException {
		 ...
		 * 
		 * @Override public int compare(MapleCharacter o1, MapleCharacter o2) {
		 * int thisVal = o1.getCheatTracker().getPoints(); int anotherVal =
		 * o2.getCheatTracker().getPoints(); return (thisVal<anotherVal ? 1 :
		...
		for (int x = allplayers.size() - 1; x >= 0; x--) {
			...
			if (cheater.getCheatTracker().getPoints() > 0) {
				cheaters.add(new CheaterData(cheater.getCheatTracker().getPoints(),
		...
		if (addChar != null) {
			...
			case ADDED:
				if (buddylist.contains(cidFrom)) {
					...
					addChar.getClient().announce(CWvsContext.FriendResult.friendChangeChannel(cidFrom, channel - 1));
				}
				break;
			case DELETED:
				if (buddylist.contains(cidFrom)) {
					buddylist.put(new BuddylistEntry(name, cidFrom, -1, buddylist.get(cidFrom).isVisible()));
					addChar.getClient().announce(CWvsContext.FriendResult.friendChangeChannel(cidFrom, -1));
				}
				break;
			}
		}
		...
		for (int characterId : recipientCharacterIds) {
			...
			if (chr != null) {
				if (chr.getBuddylist().containsVisible(cidFrom)) {
					chr.getClient().announce(CField.GroupMessage.BuddyChat.chat(nameFrom, chattext));
				}
			}
		}
	}
	...
	public void changeEmblem(int gid, List<Integer> affectedPlayers, MapleGuildSummary mgs) throws RemoteException {
		...
		this.sendPacket(affectedPlayers, MaplePacketCreator.guildEmblemChange(gid, mgs.getLogoBG(),
				mgs.getLogoBGColor(), mgs.getLogo(), mgs.getLogoColor()), -1);
	...
	public void messengerInvite(String sender, int messengerid, String target, int fromchannel) throws RemoteException {
		if (isConnected(target)) {
			...
			if (messenger == null) {
				server.getPlayerStorage().getCharacterByName(target).getClient().announce(CField.Messenger.invite(sender, messengerid));
				MapleCharacter from = ChannelServer.getInstance(fromchannel).getPlayerStorage().getCharacterByName(sender);
				from.getClient().announce(CField.Messenger.Note.inviteSent(target));
			} else {
				MapleCharacter from = ChannelServer.getInstance(fromchannel).getPlayerStorage().getCharacterByName(sender);
				from.getClient().announce(CField.Messenger.chat(sender + " : " + target + " is already using Maple Messenger"));
			}
		}
	}
			...
			if (messengerchar.getChannel() == server.getChannel() && !(messengerchar.getName().equals(namefrom))) {
				...
				if (chr != null) {
					...
					chr.getClient().getSession()
							.write(MaplePacketCreator.addMessengerPlayer(namefrom, from, position, fromchannel - 1));
					from.getClient().announce(MaplePacketCreator.addMessengerPlayer(chr.getName(), chr,
							messengerchar.getPosition(), messengerchar.getChannel() - 1));
				}
			} else
				if (messengerchar.getChannel() == server.getChannel() && (messengerchar.getName().equals(namefrom))) {
				MapleCharacter chr = server.getPlayerStorage().getCharacterByName(messengerchar.getName());
				if (chr != null) {
					chr.getClient().announce(CField.Messenger.join(messengerchar.getPosition()));
				}
			}
		}
	}
	...
	public void removeMessengerPlayer(MapleMessenger messenger, int position) throws RemoteException {
		for (MapleMessengerCharacter messengerchar : messenger.getMembers()) {
			if (messengerchar.getChannel() == server.getChannel()) {
				MapleCharacter chr = server.getPlayerStorage().getCharacterByName(messengerchar.getName());
				if (chr != null) {
					chr.getClient().announce(CField.Messenger.removePlayer(position));
				}
			}
		}
	}
	...
	public void messengerChat(MapleMessenger messenger, String chattext, String namefrom) throws RemoteException {
		for (MapleMessengerCharacter messengerchar : messenger.getMembers()) {
			if (messengerchar.getChannel() == server.getChannel() && !(messengerchar.getName().equals(namefrom))) {
				MapleCharacter chr = server.getPlayerStorage().getCharacterByName(messengerchar.getName());
				if (chr != null) {
					chr.getClient().announce(CField.Messenger.chat(chattext));
				}
			}
		}
	}
	...
	public void declineChat(String target, String namefrom) throws RemoteException {
		if (isConnected(target)) {
			MapleMessenger messenger = server.getPlayerStorage().getCharacterByName(target).getMessenger();
			if (messenger != null) {
				server.getPlayerStorage().getCharacterByName(target).getClient().announce(CField.Messenger.Note.inviteDeclined(namefrom));
			}
		}
	}
